...
![我用Excel把PS变成了一挺重机枪：一款粉橘色调的商品运营课程视频封面](https://cdn.victor42.work/posts/2024-06/d257445c788894aad2f9c9d25333d834.jpg)
...
![我用Excel把PS变成了一挺重机枪：十张不同配色与主题的课程视频封面网格](https://cdn.victor42.work/posts/2024-06/927f0f7ac6f154b4027673e30b629be2.jpg)

![我用Excel把PS变成了一挺重机枪：三种不同配色方案的课程类型标签设计](https://cdn.victor42.work/posts/2024-06/2f196e8210e1f43dae80cd978031cf36.jpg)

   ![我用Excel把PS变成了一挺重机枪：课程封面双行标题与单行标题占位设计](https://cdn.victor42.work/posts/2024-06/0a0c12d573ee7d0f2958cd0baee914fc.jpg)
   
![我用Excel把PS变成了一挺重机枪：两款不同图标与配色底栏文字设计切片](https://cdn.victor42.work/posts/2024-06/f60e4590c7b4551a14e93c37b5396f8d.jpg)
...
![我用Excel把PS变成了一挺重机枪：电子表格中整理的课程封面元数据参数表](https://cdn.victor42.work/posts/2024-06/3edc562bdfee2fb96d5271d682185f6b.jpg)
...
![我用Excel把PS变成了一挺重机枪：图像软件中包含文字与前景色的图层面板](https://cdn.victor42.work/posts/2024-06/f0990a616f2f602bcdd42a44bb01df9e.jpg)
...
![我用Excel把PS变成了一挺重机枪：电子表格维度表中分类对应的资源路径](https://cdn.victor42.work/posts/2024-06/6d4da5705eb81be2c0eee26a2cf600a7.jpg)

上面这段许多概念属于数据模型与数据库领域。简单说就是把许多属性定义在分类上。只要属于某个分类，就自动根据分类名读出对应的背景图、前景色等属性。实际需求也正是如此。

![我用Excel把PS变成了一挺重机枪：电子表格合并表中整理的课程标题和词条](https://cdn.victor42.work/posts/2024-06/a536f153f4837dc8de1937558ca0482d.jpg)

现在，运营填好的全部数据（4列）已经在我的Excel文件里了，把它引用到隔壁的事实表里，根据PS的需要加上各个运算列，构成完整的数据表。每次使用时更新数据，保存成csv格式，交给PS处理。

![我用Excel把PS变成了一挺重机枪：在电子表格中定义的封面元数据配置表](https://cdn.victor42.work/posts/2024-06/3edc562bdfee2fb96d5271d682185f6b.jpg)
...
![我用Excel把PS变成了一挺重机枪：电子表格包含各类课程字数限制的工作表](https://cdn.victor42.work/posts/2024-06/800867584af30783c478924a6db86fdd.jpg)

先来解决第1个。之前散出去给运营填的是在线表格，每人领其中几页，互不影响，且能实时更新。而我的数据表是一个本地Excel文件，因为要用到Excel强大的Power Query功能来合并表格，大多数在线表格产品没有这种能力。

![我用Excel把PS变成了一挺重机枪：系统文件夹中包含的封面模板与收集数据](https://cdn.victor42.work/posts/2024-06/521925ce776cc959e7698c66e0969042.jpg)

每当要输出一批图，首先要把在线表格以Excel文件的形式下载下来（封面图内容收集.xlsx），放到和我数据表（封面图内容.xlsx）同一个目录下。只要文件位置不动，这层数据读取的关联关系就能一直保持下去。

![我用Excel把PS变成了一挺重机枪：电子表格数据标签页中的获取数据按钮](https://cdn.victor42.work/posts/2024-06/2878de8a5b1e67e4846803c6e585a9b3.jpg)

在数据表中使用“数据”菜单里的Power Query。这个功能可以理解为一种图形化界面的SQL，它可以从本地表格、网页、本地数据库、Azure云上读取数据，并进一步清洗、转换、聚合。我这里用到的是它从本地表格读取数据的能力。

![我用Excel把PS变成了一挺重机枪：数据查询编辑器中导入的数据流与步骤](https://cdn.victor42.work/posts/2024-06/be21ed673a8ff88073305b48f847fefd.jpg)
...
![我用Excel把PS变成了一挺重机枪：数据查询编辑器左侧的所有查询列表项](https://cdn.victor42.work/posts/2024-06/af1dc0620c1d62b971ac405eddbb55cc.jpg)

其中最核心是第2步。界面左侧列表就是一个个的查询请求，它会按顺序执行下来。

每个请求里要“将第一行用作标题”，并且在筛选条件里去掉空值。

![我用Excel把PS变成了一挺重机枪：数据查询编辑器工具栏中的追加查询按钮](https://cdn.victor42.work/posts/2024-06/69054b9e157cd497d0ede7fa9d85a547.jpg)
...
![我用Excel把PS变成了一挺重机枪：数据查询编辑器追加多表数据的配置窗口](https://cdn.victor42.work/posts/2024-06/137c056d4946b2367dc3259518c0e5dd.jpg)

顺道提一下，它的合并查询也非常有用，能实现SQL里JOIN和LEFT JOIN的效果。

![我用Excel把PS变成了一挺重机枪：数据查询编辑器工具栏的关闭并上传按钮](https://cdn.victor42.work/posts/2024-06/66b4091f7dfc0ebf0448baf72b6509c0.jpg)
...
![我用Excel把PS变成了一挺重机枪：电子表格数据标签页中的全部刷新按钮](https://cdn.victor42.work/posts/2024-06/e56e2966a217111a4a10f15db546dd42.jpg)
...
![我用Excel把PS变成了一挺重机枪：图像处理软件中包含更多细节的图层面板](https://cdn.victor42.work/posts/2024-06/531b776b93a3a94233d9c2842edd7612.jpg)
...
![我用Excel把PS变成了一挺重机枪：图像处理软件中色相与饱和度调节面板](https://cdn.victor42.work/posts/2024-06/a9a688b8e6f7b88860fa0b00a4b79c1b.jpg)
...
![我用Excel把PS变成了一挺重机枪：图像处理软件中图层的变量定义配置窗口](https://cdn.victor42.work/posts/2024-06/ea2af0bf27f4c74c49db871f5c2b7296.jpg)

变量定义功能最常用的是替换文字，用到的是其中的文本替换这个选项。当变量图层不是文本时，它就会变为像素替换，也就是换图。背景图通过这种方式替换。

![我用Excel把PS变成了一挺重机枪：系统文件夹中各种颜色的纯色背景小图片](https://cdn.victor42.work/posts/2024-06/5be7d1ed67cc798dc3a2925f3e07cc84.jpg)

前景色也一样，为每种文字颜色准备相应的纯色图片，把上一步创建的剪贴模板图层定义为变量，根据课程分类选用相应颜色。

![我用Excel把PS变成了一挺重机枪：电子表格中控制图层可见性的逻辑判断列](https://cdn.victor42.work/posts/2024-06/46b0372dc8bd46ffed011eae85e6bb28.jpg)

可见性变量常被忽视，但非常有用。表格里这些TRUE和FALSE的列，就是通过可见性来控制图层的显示隐藏。可见性变量还可以和文本替换或像素替换同时使用。比如底部描述小字，文本替换改变它的内容，可见性则控制它显示隐藏。

![我用Excel把PS变成了一挺重机枪：图像处理软件中配置描述变量的设置窗口](https://cdn.victor42.work/posts/2024-06/a740f3312e02a860493537f9e78b80c0.jpg)

前两步的工作虽然繁琐，但都是一次性的，在后续的使用中不必重复。

### 3. 导入数据组

![我用Excel把PS变成了一挺重机枪：设计软件数据组管理面板中高亮的导入按钮](https://cdn.victor42.work/posts/2024-06/e1fbcc63e3a13d31e181c8a1f12db265.jpg)

切换到导入面板，把准备好的csv文件导进来就是了。

导入常见的错误有两种，一种是有多余的列或名称对不上的列，需要检查。

![我用Excel把PS变成了一挺重机枪：电子表格中用于标记空值单元格的空值字符](https://cdn.victor42.work/posts/2024-06/6c7cabdd58fab84c63dfefdfeb4ad16a.jpg)

另一种是有的行里有空单元格。是的，PS导入数据组不支持空单元格，所以我在制作数据表的时候把空单元格内容都改成了NULL。这些NULL显示出来会破坏图片效果，所以可能为空的列都要做可见性判断，适时隐藏。

### 4. 批量导出psd

![我用Excel把PS变成了一挺重机枪：设计软件中将数据组导出为文件的菜单](https://cdn.victor42.work/posts/2024-06/d92857c1843b212c5205b9d0582f88b2.jpg)

导出没什么技巧，这么操作就是了。

![我用Excel把PS变成了一挺重机枪：图像处理软件中配置数据组导出命名规则](https://cdn.victor42.work/posts/2024-06/7dceeaca255670e1437a37b68e773e31.jpg)
...
![我用Excel把PS变成了一挺重机枪：图像处理软件中录制的另存为动作面板](https://cdn.victor42.work/posts/2024-06/c36d844e7381a77f1c02718a560fe9f8.jpg)
...
![我用Excel把PS变成了一挺重机枪：统计封面图数量及产出价值的数据统计表](https://cdn.victor42.work/posts/2024-06/fa9ce9f2b382b99c6cad7125d176799b.jpg)
