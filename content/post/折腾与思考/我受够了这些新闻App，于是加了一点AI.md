...
![我受够了这些新闻App，于是加了一点AI：任务控制中用于读取网页链接的配置页面](https://cdn.victor42.work/posts/2024-08/7c717fff18ada6917cc6ddb9ab5acab4.jpg)
...
![我受够了这些新闻App，于是加了一点AI：第一财经网页新闻源中的标准代码结构](https://cdn.victor42.work/posts/2024-08/d9b55eddd9d7d7e26b86fffe3dc56a1f.jpg)
...
![我受够了这些新闻App，于是加了一点AI：用于规整代码格式和替换转义字符的任务](https://cdn.victor42.work/posts/2024-08/c117f68d572158b87dc54acd03427ad3.jpg)
...
![我受够了这些新闻App，于是加了一点AI：提取代码中所有相邻特定标签内容的方法](https://cdn.victor42.work/posts/2024-08/97c487fda93b7ef67d871a58ebd06721.jpg)
...
![我受够了这些新闻App，于是加了一点AI：从复杂网页代码中提取首个匹配标签内容](https://cdn.victor42.work/posts/2024-08/0f45bc228aba2af149ceeb0c69a67907.jpg)
...
![我受够了这些新闻App，于是加了一点AI：利用系统自带功能将网页代码转换为纯文本](https://cdn.victor42.work/posts/2024-08/75b7ba16f35c56d3bb121026e9098eeb.jpg)
...
![我受够了这些新闻App，于是加了一点AI：配置调用大语言模型进行分类的请求任务](https://cdn.victor42.work/posts/2024-08/05c6c50a2cba1cad021f550344301002.jpg)
...
![我受够了这些新闻App，于是加了一点AI：批量将已分类新闻文本合成为音频的任务](https://cdn.victor42.work/posts/2024-08/c1dabc63612d30a30e611297c14b6493.jpg)

关键步骤用到了Tasker自带的Say To File，文本存为音频文件。需要注意的是，Say To File只是提供了这种操作，合成过程需要的语音合成引擎，Tasker并没有内置。

![我受够了这些新闻App，于是加了一点AI：应用商店中谷歌语音识别与合成软件页面](https://cdn.victor42.work/posts/2024-08/995e0fb18649e334014f111ea8be2b8d.jpg)

我用了谷歌的本地语音合成引擎，Google Play下载这个App，就能在Tasker里调用。

![我受够了这些新闻App，于是加了一点AI：系统设置中选择默认语音合成引擎的弹窗](https://cdn.victor42.work/posts/2024-08/07e0c2612c68af093e0a4e5c942ab102.jpg)
...
![我受够了这些新闻App，于是加了一点AI：下载并筛选特定新闻核心任务的初始阶段](https://cdn.victor42.work/posts/2024-08/64acc1961ddd336d05f0b9aba63739ec.jpg)

从新闻源获得完整XML代码，把转义字符规整成标准XML，去掉一些特殊的内容标记。然后提取新闻列表。

![我受够了这些新闻App，于是加了一点AI：新闻分类下载任务中对列表进行数组拆分](https://cdn.victor42.work/posts/2024-08/b52349abcd5a92bb918c797c3043868a.jpg)

新闻列表根据分隔符分成数组，设定好AI提示词，设定正文长度上限（过滤掉太长的正文）。开始循环，每条新闻从XML里读出标题，标题转成纯文本，交给AI分类。

![我受够了这些新闻App，于是加了一点AI：在核心任务中调用智能对话分类器的参数](https://cdn.victor42.work/posts/2024-08/ba4c7edd6576c053b69d271d37f2bd88.jpg)

AI的提示词我是这么写的，没用到什么技巧，直白说出需求就行。由于这里处理的都是中文信息，Groq上的Gemma2 9b模型比较适合，比Llama3.1的中文能力强。这种简单需求，开源小模型足以胜任。实际使用效果很好，没出过错。

![我受够了这些新闻App，于是加了一点AI：根据分类结果过滤并将正文写入文本文件](https://cdn.victor42.work/posts/2024-08/3e5a40b3c15e4e3661f026de131b45f0.jpg)
...
![我受够了这些新闻App，于是加了一点AI：系统帮助文档中关于任务执行优先级说明](https://cdn.victor42.work/posts/2024-08/9fe1b7073ff0b94fab2859978f94ec9f.jpg)
...
![我受够了这些新闻App，于是加了一点AI：在子任务配置中调整优先级参数防止冲突](https://cdn.victor42.work/posts/2024-08/1091748819015f7296ec93d7500e5475.jpg)
...
![我受够了这些新闻App，于是加了一点AI：用于调用多个不同渠道新闻源的主体任务](https://cdn.victor42.work/posts/2024-08/a0a90a2ca998bca90156e3cfe59040b5.jpg)

把我选出的几个RSS新闻源传递给核心任务，从哪里取正文也告诉它。每个新闻源都执行一次。

![我受够了这些新闻App，于是加了一点AI：执行批量语音合成并将音频导出的控制流](https://cdn.victor42.work/posts/2024-08/d6d37dbe8a8195e2d6c7674104fc533f.jpg)
...
![我受够了这些新闻App，于是加了一点AI：设置在每天凌晨四点触发自动下载的配置文件](https://cdn.victor42.work/posts/2024-08/bf1751cc5b2863826ff82d819e8b8859.jpg)

每天凌晨4点，把新闻都存成文本文件。这个过程要5-10分钟。

![我受够了这些新闻App，于是加了一点AI：设置在每天凌晨五点触发语音合成的配置文件](https://cdn.victor42.work/posts/2024-08/e1f5ef475b315060c9a3679f7a0e0603.jpg)
...
![我受够了这些新闻App，于是加了一点AI：文件管理器中存放文本和音频新闻的文件夹](https://cdn.victor42.work/posts/2024-08/7add1606a97bddcc6fdee7af42f71cb1.jpg)

text保存了文字版新闻，如果有需要我还能二次分享出去。

![我受够了这些新闻App，于是加了一点AI：文件管理器中已保存的每日新闻文本列表](https://cdn.victor42.work/posts/2024-08/4d00497b3a8e5554ff90aeccfe11dcbd.jpg)

audio文件夹里是音频新闻。虽然还有一些没什么意思的社会新闻混在其中，但这不能怪AI，至少我再也没有听到过体育新闻了。

![我受够了这些新闻App，于是加了一点AI：音乐播放器中选中的每日新闻播放列表](https://cdn.victor42.work/posts/2024-08/fb2a13c2d652d15b0653f2e39be0beea.jpg)

手机上的音乐播放器里新建了一个叫每日新闻的歌单，专门读取audio文件夹。

![我受够了这些新闻App，于是加了一点AI：播放器主界面中展示的当天新闻音频列表](https://cdn.victor42.work/posts/2024-08/90415a2f2e2fa7829fae2a10f117f392.jpg)

更新一下内容，当天新闻就都来了。这个更新过程仍然需要手动点一下，我还在找自动化的办法。

![我受够了这些新闻App，于是加了一点AI：车载蓝牙连接时自动播放新闻的配置文件](https://cdn.victor42.work/posts/2024-08/0023ce1bc26cb0c58b78cab5d834c033.jpg)
