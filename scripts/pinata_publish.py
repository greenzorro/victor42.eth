...
def list_workflow_pins(jwt):
    ...
    while True:
        ...
        payload = pinata_request(jwt, "GET", "/v3/files/public", query=query)
        ...
        page_token = payload.get("data", {}).get("next_page_token")
        if not page_token:
            break
...
def delete_pinata_files(jwt, file_ids):
    for file_id in file_ids:
        pinata_request(jwt, "DELETE", f"/v3/files/public/{urllib.parse.quote(file_id, safe='')}")
...
def cleanup_old_pins(jwt, current_cid, keep, dry_run):
    ...
    pins = sorted(
        ...
        key=lambda item: parse_pinata_timestamp(item.get("created_at") or item.get("date_pinned")),
        reverse=True,
    )
    keep_ids = {item["id"] for item in pins[:keep] if item.get("id")}
    keep_ids.update(item["id"] for item in pins if item.get("cid") == current_cid and item.get("id"))
    delete_candidates = [item for item in pins if item.get("id") not in keep_ids]

    print(f"PINATA_RETENTION_TOTAL={len(pins)}")
    print(f"PINATA_RETENTION_KEEP={len(keep_ids)}")
    print(f"PINATA_RETENTION_DELETE={len(delete_candidates)}")
    if not delete_candidates:
        return

    for item in delete_candidates:
        print(f"PINATA_RETENTION_DELETE_CANDIDATE={item.get('id')} cid={item.get('cid')}")

    if dry_run:
        print("PINATA_RETENTION_DRY_RUN=true")
        return

    delete_pinata_files(jwt, [item["id"] for item in delete_candidates if item.get("id")])
...
def main():
    ...
    data = pin.get("data", pin)
    print(f"PINATA_PIN_ID={data.get('id')}")
    print(f"PINATA_PIN_CID={data.get('cid')}")
    print(f"PINATA_PIN_STATUS={data.get('status')}")
